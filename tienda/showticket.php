...
$adatos = recuperarInfoTickets($idSesion,$id_ticket);

function recuperarInfoTickets($idSesion,$id_ticket){
    $resultado = array();
    $bd = abrirConexion();
    $operadorSelect=mysqli_query($bd, "SELECT
            detallecarritousuario.id,
			carrito_usuarios.id_sesion,
			carrito_usuarios.totalproducto,
			carrito_usuarios.fecha,
            detallecarritousuario.idProducto_fk,
            detallecarritousuario.precio,
            detallecarritousuario.cantidad,
            detallecarritousuario.subtotal,
            productos.nombre
            FROM carrito_usuarios 
            JOIN detallecarritousuario ON carrito_usuarios.id = detallecarritousuario.idCarrito_FK
            JOIN productos ON detallecarritousuario.idProducto_fk = productos.id_producto
            WHERE carrito_usuarios.id_sesion='$idSesion'
            AND detallecarritousuario.idCarrito_FK='$id_ticket'
            ");
    while($row=mysqli_fetch_array($operadorSelect)){
            ...
            $index['fecha']=$row[3];
            $index['idProducto_fk']=$row[4];
            $index['precio']=$row[5];
            $index['cantidad']=$row[6];
