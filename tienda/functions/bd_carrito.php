...
$GLOBALS["contrasena"] = "";
$GLOBALS["base_datos"] = "tienda";
$GLOBALS["raiz_sitio"] = "http://localhost/tienda/";
function abrirConexion()
{
  $pconector = mysqli_connect($GLOBALS["servidor"], $GLOBALS["usuario"], $GLOBALS["contrasena"], $GLOBALS["base_datos"]) or die(mysqli_connect_error());
  return $pconector;
}
...
function contarFilas($pconector, $cquery)
{
  $lresult = mysqli_query($pconector, $cquery);
  if (!$lresult) {
    $cerror = "Ocurri&oacute; un error al acceder a la base de datos.<br>";
    ...
    $total_filas = mysqli_num_rows($lresult);
    return $total_filas;
  }
  mysqli_free_result($lresult);
  reset($aregistro);
